// First-person controller: pointer lock, WASD, circle-vs-AABB collision,
// floor-height regions (for the staircase) and a subtle head bob.
...
export class Controls {
  ...
  update(dt){
    if (!this.enabled || this.frozen){ this._applyCamera(0); return; }
    ...
    this._tryMove(move.x, 0, floorBefore);
    this._tryMove(0, move.z, floorBefore);
    ...
    // head bob + footsteps
    if (this.moving){
      this.bobT += dt * (fast ? 9.4 : 7.0);
      this._stepAcc += dt * (fast ? 2.4 : 1.75);
      if (this._stepAcc >= 1){
        this._stepAcc = 0;
    ...
    this._applyCamera(this.moving ? Math.sin(this.bobT) * 0.028 : 0);
  }
